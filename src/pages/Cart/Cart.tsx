      ...
      <div className='grid grid-cols-12 gap-6'>
            ...
            {items.map((it) => (
              <div key={it.id} className='grid grid-cols-12 items-center gap-4 bg-white p-4 border border-gray-100 rounded'>
                <div className='col-span-12 md:col-span-6 flex items-center gap-4'>
                  <img src={it.image || ''} alt={it.productName} className='w-20 h-20 object-cover rounded' />
                  <div>
                    <h3 className='font-medium'>{it.productName}</h3>
                    <p className='text-sm text-gray-600 hidden md:block'>Product description or short details</p>
                  </div>
                </div>
                ...
                <div className='col-span-6 md:col-span-2 text-right font-semibold flex items-center justify-end gap-3'>
                  <div>${((it.discountPrice && it.hasDiscount ? it.discountPrice : it.price ?? 0) * it.quantity).toFixed(2)}</div>
                  <button onClick={() => dispatch(removeFromCart(it.id))} className='w-7 h-7 flex items-center justify-center rounded-full bg-red-100 text-red-500'>✕</button>
                </div>
              </div>
            ))}
            ...
            {items.length > 0 && (
              <div className='flex items-center justify-between mt-6'>
                <div className='flex items-center gap-3'>
                  <button onClick={() => navigate('/')} className='px-6 py-2 border rounded'>Return To Shop</button>
                  <button onClick={() => { /* could trigger update logic */ }} className='px-6 py-2 border rounded'>Update Cart</button>
                  <button onClick={() => dispatch(clearCart())} className='px-6 py-2 border border-red-300 text-red-500 rounded'>Remove all</button>
                </div>

                <div className='flex items-center gap-3'>
                  <input placeholder='Coupon Code' className='px-4 py-2 border rounded' />
                  <button className='px-4 py-2 bg-red-100 text-red-500 rounded'>Apply</button>
                </div>
              </div>
            )}
        ...
        <div className='col-span-4'>
          <div className='p-6 border rounded bg-white'>
            ...
            <div className='flex items-center justify-between mb-2'><span>Subtotal:</span><span>${subtotal.toFixed(2)}</span></div>
            <div className='flex items-center justify-between mb-4'><span>Shipping:</span><span>Free</span></div>
            ...
            <button onClick={() => navigate('/checkout')} className='w-full bg-red-500 text-white py-3 rounded'>Proceeds to checkout</button>
          </div>
        </div>
      </div>
    </div>
