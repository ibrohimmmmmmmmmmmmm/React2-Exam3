    ...
    <div className="px-4 sm:px-16 py-8 max-w-[1200px] mx-auto">
      ...
      <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between gap-8 sm:gap-16 mb-12 sm:mb-24">
        <div className="w-full sm:max-w-[450px]">
          <h1 className="text-[36px] sm:text-[54px] font-semibold leading-tight mb-6 sm:mb-8">Our Story</h1>
        ...
        <div className="w-full sm:flex-1">
          <img
            ...
            alt="Shopping"
            className="w-full h-[280px] sm:h-[420px] object-cover rounded-sm"
          />
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-4 gap-4 sm:gap-6 mb-12 sm:mb-24">
        {stats.map((s, i) => (
          <div
            key={i}
            className={`flex flex-col items-center justify-center py-8 sm:py-10 px-6 rounded border transition-all cursor-default
              ${s.active
          ...
          >
            <div className={`w-[60px] h-[60px] rounded-full flex items-center justify-center mb-4
              ${s.active ? "bg-white/30" : "bg-gray-100"}`}
            >
              {s.icon}
            </div>
            <span className="text-[28px] sm:text-[32px] font-semibold mb-1">{s.value}</span>
            <span className="text-sm text-center opacity-80">{s.label}</span>
          </div>
        ))}
      </div>

      {/* TEAM SWIPER — 1 slide mobile, 3 desktop */}
      <div className="mb-12 sm:mb-24">
        <Swiper
          ...
          spaceBetween={16}
          className="teamSwiper pb-12"
        >
          {team.map((member, i) => (
            <SwiperSlide key={i}>
              <div>
                <div className="bg-gray-100 rounded-sm overflow-hidden mb-4">
                  <img
                    src={member.image}
                    alt={member.name}
                ...
                <h3 className="text-[20px] sm:text-[22px] font-semibold mb-1">{member.name}</h3>
                <p className="text-gray-500 text-sm mb-3">{member.role}</p>
                <div className="flex items-center gap-3">
                  <a href="#" className="hover:text-[#DB4444] transition"><TwitterIcon size={18} /></a>
                  <a href="#" className="hover:text-[#DB4444] transition"><InstagramIcon size={18} /></a>
                  <a href="#" className="hover:text-[#DB4444] transition"><LinkedinIcon size={18} /></a>
                </div>
              </div>
            </SwiperSlide>
          ))}
      ...
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-10 sm:gap-8 mb-16">
        {features.map((f, i) => (
          <div key={i} className="flex flex-col items-center text-center">
            <div className="w-[70px] h-[70px] rounded-full bg-gray-200 flex items-center justify-center mb-4">
              <div className="w-[50px] h-[50px] rounded-full bg-black flex items-center justify-center text-white">
                {f.icon}
              </div>
            </div>
            <h4 className="font-semibold text-[14px] tracking-wide mb-2">{f.title}</h4>
